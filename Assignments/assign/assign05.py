...
SCORE_HIT = 1
# this has been changed from 5 to 2 by Yurii Vasiuk
SCORE_MISS = 2

"""
This is a set of my classes (Point, Velocity, Ball, Paddle)
The classes are used by Pong
"""

# just the coordinates for the ball
class Point:
    def __init__(self, x = 0, y = 0):
        self.x = x
        self.y = y

# the numbers for velocity and acceleration which will be called in Pong()->check_hit()
class Velocity:
    def __init__(self):
        self.dx = 5
        self.dy = 5
...
# the ball with Point() coordinates, Velocity(), and methods to draw, and to move and check it
class Ball:
    def __init__(self):
        self.center = Point()
        self.restart()
    ...
    def draw(self):
        ...
        # this will draw the current velocity
        score_text = "Velocity: {}".format(abs(self.velocity.dy))
        start_x = 10
...
# handle the paddle with limiting its up and down movements
class Paddle:
    def __init__(self):
        self.center = Point(SCREEN_WIDTH - PADDLE_WIDTH // 2, SCREEN_HEIGHT // 2)
...
class Pong(arcade.Window):
    ...
    def check_hit(self):
        ...
        if (abs(self.ball.center.x - self.paddle.center.x) < too_close_x and
            ...
            # added by Yurii Vasiuk for accelerating the ball every 3 positive hits
            if self.score > 0 and self.score % 3 == 0:
                self.ball.velocity.accelerate()
