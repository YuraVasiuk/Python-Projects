...
class MyApplication(arcade.Window):
    ...
    def on_draw(self):
        ...
        if(self.space == True):
            arcade.draw_circle_filled(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2,
                                      BALL_RADIUS, arcade.color.BLUE)

        if(self.print_speed == True):
            arcade.draw_text("Speed = {}".format(self.ball_x_pixels_per_second),
                             10, SCREEN_HEIGHT // 5, arcade.color.BLUE, 20)
    ...
    def on_mouse_press(self, x, y, button, key_modifiers):
        ...
        if(self.ball_x_pixels_per_second > 0):
            self.ball_x_pixels_per_second += 20
        else:
            self.ball_x_pixels_per_second -= 20
        self.print_speed = True
